...
module.exports = async function(payload, options) {
  ...
  if (!details.disableTracking && settings.enableMetrics) {
    ...
    const toStr = (Array.isArray(details.to)) ? details.to.join('|') : details.to.replace(',', '|');
    details.data.trackingPixel = `<img src="${settings.metricsHost}t.gif?type=email.open&value=1&tags=${tags}&fields=toEmail:${toStr},uuid:${details.uuid}"></img>`;
  } else {
